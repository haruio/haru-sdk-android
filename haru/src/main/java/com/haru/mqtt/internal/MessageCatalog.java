		...
		if (INSTANCE == null) {
			if (ExceptionHelper.isClassAvailable("java.util.ResourceBundle")) {
				try {
					...
					INSTANCE = (MessageCatalog)Class.forName("com.haru.mqtt.internal.ResourceBundleCatalog").newInstance();
				} catch (Exception e) {
					return "";
				}
			} else if (ExceptionHelper.isClassAvailable("com.haru.mqtt.internal.MIDPCatalog")){
				try {
					INSTANCE = (MessageCatalog)Class.forName("com.haru.mqtt.internal.MIDPCatalog").newInstance();
				} catch (Exception e) {
					return "";
				}
			}
